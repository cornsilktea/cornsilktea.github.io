...
- **게임 시작 화면은 30·31번 모양으로 만든다.** 게임 화면 위에 어두운 반투명 판(`theme.css` 의 `st-*` 클래스:
  ...
  **모든 게임은 초기화면뿐 아니라 대기화면(모드·코스·차량 선택, 방 만들기·참여, 방 대기실, 결과)까지 30·31번처럼
  그 게임 테마에 맞춘 어두운 반투명 카드로 만든다.** 흰 카드가 섞이지 않게 한다(24·25번 `#overlay.st-screen .card` 참고).
  게임 중 HUD 는 24번 터보 랩처럼 어두운 반투명 상자(`.hud-box`)·흰 글씨·주황(`#D97B4F`) 강조선으로 맞춘다(27~29번 참고).
  이모지는 윈도우 10·갤럭시탭에 없는 최신 것(🪙 등)을 쓰지 않는다 — 깨져 보인다.
